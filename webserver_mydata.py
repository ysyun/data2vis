...
from utils import data_utils_mydata
import argparse
...
def examplesdata():
    source_data = data_utils_mydata.load_test_dataset()
    f_names = data_utils_mydata.generate_field_types(source_data)
    data_utils_mydata.forward_norm(source_data, destination_file, f_names)
    ...
    #     decoded_post = data_utils_mydata.backward_norm(row, f_names)
    #     decoded_post_array.append(decoded_post)

    decoded_string_post = data_utils_mydata.backward_norm(decoded_string[0], f_names)
    print('>>>> 2: ')
...
@app.route("/testdata")
def testdata():
    return jsonify(data_utils_mydata.load_test_dataset())
...
def inference():
    ...
    f_names = data_utils_mydata.generate_field_types(source_data)
    fnorm_result = data_utils_mydata.forward_norm(source_data, destination_file,
                                           f_names)
    ...
    # # Perform post processing - backward normalization
    # decoded_string_post = data_utils_mydata.backward_norm(decoded_string, f_names)
